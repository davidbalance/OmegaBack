import { ResourceModel as PrismaModel } from "@prisma/client";
import { ResourceModelPrismaRepository } from "../resource.prisma-respository";
...
import { ResourceModelMapper } from "@omega/adapter/persistence/prisma/mapper/auth/model/resource.model-mapper";
import { RepositoryError } from "@shared/shared/domain/error";

describe('ResourceModelPrismaRepository', () => {
    let prisma;
    let repository: ResourceModelPrismaRepository;
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ResourceModelPrismaRepository,
                { provide: PrismaService, useValue: prisma }
            ]
        }).compile();

        repository = module.get<ResourceModelPrismaRepository>(ResourceModelPrismaRepository);
    });
    ...
    describe('findManyAsync', () => {
        const mockFilter: SearchCriteria<ResourceModel> = { filter: [{ field: 'resourceId', operator: 'eq', value: 'value1' }] };
        ...
        it('should return mapped values when Prisma returns results', async () => {
            ...
            const modelResult = { id: 'id-123' } as unknown as ResourceModel;

            prisma.resourceModel.findMany.mockResolvedValue(prismaResults);
            jest.spyOn(ResourceModelMapper, "toModel").mockReturnValue(modelResult);
            ...
            expect(prisma.resourceModel.findMany).toHaveBeenCalledWith({
        ...
        it('should return empty array when Prisma returns no results', async () => {
            prisma.resourceModel.findMany.mockResolvedValue([]);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.resourceModel.findMany.mockRejectedValue(new Error('DB error'));
    ...
    describe('findOneAsync', () => {
        const mockFilter: Filter<ResourceModel>[] = [{ field: 'resourceId', operator: 'eq', value: 'value1' }];
        const mockPrismaWhere = { name: 'value1' };
        ...
        it('should return mapped value when Prisma returns result', async () => {
            ...
            prisma.resourceModel.findFirst.mockResolvedValue(prismaResult);
            jest.spyOn(ResourceModelMapper, "toModel").mockReturnValue(modelResult);

            const result = await repository.findOneAsync(mockFilter);

            expect(prisma.resourceModel.findFirst).toHaveBeenCalledWith({ where: mockPrismaWhere });
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.resourceModel.findFirst.mockRejectedValue(new Error('Oops'));
