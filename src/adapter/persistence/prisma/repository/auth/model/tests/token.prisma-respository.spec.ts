import { TokenModel as PrismaModel } from "@prisma/client";
import { TokenModelPrismaRepository } from "../token.prisma-respository";
...
import { TokenModelMapper } from "@omega/adapter/persistence/prisma/mapper/auth/model/token.model-mapper";
import { RepositoryError } from "@shared/shared/domain/error";

describe('TokenModelPrismaRepository', () => {
    let prisma;
    let repository: TokenModelPrismaRepository;
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                TokenModelPrismaRepository,
                { provide: PrismaService, useValue: prisma }
            ]
        }).compile();

        repository = module.get<TokenModelPrismaRepository>(TokenModelPrismaRepository);
    });
    ...
    describe('findManyAsync', () => {
        const mockFilter: SearchCriteria<TokenModel> = { filter: [{ field: 'authId', operator: 'eq', value: 'value1' }] };
        ...
        it('should return mapped values when Prisma returns results', async () => {
            ...
            const modelResult = { id: 'id-123' } as unknown as TokenModel;

            prisma.tokenModel.findMany.mockResolvedValue(prismaResults);
            jest.spyOn(TokenModelMapper, "toModel").mockReturnValue(modelResult);
            ...
            expect(prisma.tokenModel.findMany).toHaveBeenCalledWith({ where: mockPrismaWhere });
        ...
        it('should return empty array when Prisma returns no results', async () => {
            prisma.tokenModel.findMany.mockResolvedValue([]);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.tokenModel.findMany.mockRejectedValue(new Error('DB error'));
    ...
    describe('findOneAsync', () => {
        const mockFilter: Filter<TokenModel>[] = [{ field: 'authId', operator: 'eq', value: 'value1' }];
        const mockPrismaWhere = { name: 'value1' };
        ...
        it('should return mapped value when Prisma returns result', async () => {
            ...
            prisma.tokenModel.findFirst.mockResolvedValue(prismaResult);
            jest.spyOn(TokenModelMapper, "toModel").mockReturnValue(modelResult);

            const result = await repository.findOneAsync(mockFilter);

            expect(prisma.tokenModel.findFirst).toHaveBeenCalledWith({ where: mockPrismaWhere });
            expect(result).toBe(modelResult);
        });

        it('should return null when Prisma returns null', async () => {
            prisma.tokenModel.findFirst.mockResolvedValue(null);
            expect(PrismaFilterMapper.map).toHaveBeenCalledWith(mockFilter);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.tokenModel.findFirst.mockRejectedValue(new Error('Oops'));
