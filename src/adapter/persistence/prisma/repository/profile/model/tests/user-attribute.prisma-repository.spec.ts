...
import { UserAttributeModel as PrismaModel } from "@prisma/client";
...
import { UserAttributePrismaRepository } from "../user-attribute.prisma-repository";
import { UserAttributeModel } from "@omega/profile/core/model/user/user-attribute.model";
import { UserAttributeModelMapper } from "@omega/adapter/persistence/prisma/mapper/profile/model/user-attribute.model-mapper";

describe('UserAttributePrismaRepository', () => {
    let prisma;
    let repository: UserAttributePrismaRepository;
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                UserAttributePrismaRepository,
                { provide: PrismaService, useValue: prisma }
            ]
        }).compile();

        repository = module.get<UserAttributePrismaRepository>(UserAttributePrismaRepository);
    });
    ...
    describe('findManyAsync', () => {
        const mockFilter: SearchCriteria<UserAttributeModel> = { filter: [] };
        ...
        it('should return mapped values when Prisma returns results', async () => {
            ...
            const modelResult = { id: 'id-123' } as unknown as UserAttributeModel;

            prisma.userAttributeModel.findMany.mockResolvedValue(prismaResults);
            jest.spyOn(UserAttributeModelMapper, "toModel").mockReturnValue(modelResult);
            ...
            expect(prisma.userAttributeModel.findMany).toHaveBeenCalledWith({ where: mockPrismaWhere });
        ...
        it('should return empty array when Prisma returns no results', async () => {
            prisma.userAttributeModel.findMany.mockResolvedValue([]);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.userAttributeModel.findMany.mockRejectedValue(new Error('DB error'));
    ...
    describe('findOneAsync', () => {
        const mockFilter: Filter<UserAttributeModel>[] = [];
        const mockPrismaWhere = { name: 'value1' };
        ...
        it('should return mapped value when Prisma returns result', async () => {
            ...
            prisma.userAttributeModel.findFirst.mockResolvedValue(prismaResult);
            jest.spyOn(UserAttributeModelMapper, "toModel").mockReturnValue(modelResult);

            const result = await repository.findOneAsync(mockFilter);

            expect(prisma.userAttributeModel.findFirst).toHaveBeenCalledWith({ where: mockPrismaWhere });
            expect(result).toBe(modelResult);
        });

        it('should return null when Prisma returns null', async () => {
            prisma.userAttributeModel.findFirst.mockResolvedValue(null);
            expect(PrismaFilterMapper.map).toHaveBeenCalledWith(mockFilter);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.userAttributeModel.findFirst.mockRejectedValue(new Error('Oops'));
