import { DoctorOptionModel as PrismaModel } from "@prisma/client";
...
import { DoctorOptionPrismaRepository } from "../doctor-option.prisma-repository";
import { DoctorOptionModel } from "@omega/profile/core/model/user/doctor-option.model";
import { DoctorOptionModelMapper } from "@omega/adapter/persistence/prisma/mapper/profile/model/doctor-option.model-mapper";

describe('DoctorOptionPrismaRepository', () => {
    let prisma: { doctorOptionModel: any; };
    let repository: DoctorOptionPrismaRepository;
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                DoctorOptionPrismaRepository,
                { provide: PrismaService, useValue: prisma }
            ]
        }).compile();

        repository = module.get<DoctorOptionPrismaRepository>(DoctorOptionPrismaRepository);
    });
    ...
    describe('findManyAsync', () => {
        const mockFilter: SearchCriteria<DoctorOptionModel> = { filter: [{ field: 'doctorLabel', operator: 'eq', value: 'value1' }] };
        ...
        it('should return mapped values when Prisma returns results', async () => {
            ...
            const modelResult = { id: 'id-123' } as unknown as DoctorOptionModel;

            prisma.doctorOptionModel.findMany.mockResolvedValue(prismaResults);
            jest.spyOn(DoctorOptionModelMapper, "toModel").mockReturnValue(modelResult);
            ...
            expect(prisma.doctorOptionModel.findMany).toHaveBeenCalledWith({ where: mockPrismaWhere });
        ...
        it('should return empty array when Prisma returns no results', async () => {
            prisma.doctorOptionModel.findMany.mockResolvedValue([]);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.doctorOptionModel.findMany.mockRejectedValue(new Error('DB error'));
    ...
    describe('findOneAsync', () => {
        const mockFilter: Filter<DoctorOptionModel>[] = [{ field: 'doctorLabel', operator: 'eq', value: 'value1' }];
        const mockPrismaWhere = { name: 'value1' };
        ...
        it('should return mapped value when Prisma returns result', async () => {
            ...
            prisma.doctorOptionModel.findFirst.mockResolvedValue(prismaResult);
            jest.spyOn(DoctorOptionModelMapper, "toModel").mockReturnValue(modelResult);

            const result = await repository.findOneAsync(mockFilter);

            expect(prisma.doctorOptionModel.findFirst).toHaveBeenCalledWith({ where: mockPrismaWhere });
            expect(result).toBe(modelResult);
        });

        it('should return null when Prisma returns null', async () => {
            prisma.doctorOptionModel.findFirst.mockResolvedValue(null);
            expect(PrismaFilterMapper.map).toHaveBeenCalledWith(mockFilter);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.doctorOptionModel.findFirst.mockRejectedValue(new Error('Oops'));
