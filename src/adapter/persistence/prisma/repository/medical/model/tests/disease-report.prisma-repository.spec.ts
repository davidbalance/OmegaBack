import { DiseaseReportModel as PrismaModel } from "@prisma/client";
...
import { DiseaseReportPrismaRepository } from "../disease-report.prisma-repository";
import { DiseaseReportModel } from "@omega/medical/core/model/test/disease-report.model";
import { DiseaseReportModelMapper } from "@omega/adapter/persistence/prisma/mapper/medical/model/disease-report.model-mapper";

describe('DiseaseReportPrismaRepository', () => {
    let prisma: { diseaseReportModel: any; };
    let repository: DiseaseReportPrismaRepository;
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                DiseaseReportPrismaRepository,
                { provide: PrismaService, useValue: prisma }
            ]
        }).compile();

        repository = module.get<DiseaseReportPrismaRepository>(DiseaseReportPrismaRepository);
    });
    ...
    describe('findManyAsync', () => {
        const mockFilter: SearchCriteria<DiseaseReportModel> = { filter: [{ field: 'diseaseReportId', operator: 'eq', value: 'value1' }] };
        ...
        it('should return mapped values when Prisma returns results', async () => {
            ...
            const modelResult = { id: 'id-123' } as unknown as DiseaseReportModel;

            prisma.diseaseReportModel.findMany.mockResolvedValue(prismaResults);
            jest.spyOn(DiseaseReportModelMapper, "toModel").mockReturnValue(modelResult);
            ...
            expect(prisma.diseaseReportModel.findMany).toHaveBeenCalledWith({ where: mockPrismaWhere });
        ...
        it('should return empty array when Prisma returns no results', async () => {
            prisma.diseaseReportModel.findMany.mockResolvedValue([]);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.diseaseReportModel.findMany.mockRejectedValue(new Error('DB error'));
    ...
    describe('findOneAsync', () => {
        const mockFilter: Filter<DiseaseReportModel>[] = [{ field: 'diseaseReportId', operator: 'eq', value: 'value1' }];
        const mockPrismaWhere = { name: 'value1' };
        ...
        it('should return mapped value when Prisma returns result', async () => {
            ...
            prisma.diseaseReportModel.findFirst.mockResolvedValue(prismaResult);
            jest.spyOn(DiseaseReportModelMapper, "toModel").mockReturnValue(modelResult);

            const result = await repository.findOneAsync(mockFilter);

            expect(prisma.diseaseReportModel.findFirst).toHaveBeenCalledWith({ where: mockPrismaWhere });
            expect(result).toBe(modelResult);
        });

        it('should return null when Prisma returns null', async () => {
            prisma.diseaseReportModel.findFirst.mockResolvedValue(null);
            expect(PrismaFilterMapper.map).toHaveBeenCalledWith(mockFilter);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.diseaseReportModel.findFirst.mockRejectedValue(new Error('Oops'));
