import { OrderYearModel as PrismaModel } from "@prisma/client";
...
import { OrderYearPrismaRepository } from "../order-year.prisma-repository";
import { OrderYearModel } from "@omega/medical/core/model/order/order-year.model";
import { OrderYearModelMapper } from "@omega/adapter/persistence/prisma/mapper/medical/model/order-year.model-mapper";

describe('OrderYearPrismaRepository', () => {
    let prisma: { orderYearModel: any; };
    let repository: OrderYearPrismaRepository;
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                OrderYearPrismaRepository,
                { provide: PrismaService, useValue: prisma }
            ]
        }).compile();

        repository = module.get<OrderYearPrismaRepository>(OrderYearPrismaRepository);
    });
    ...
    describe('findManyAsync', () => {
        const mockFilter: SearchCriteria<OrderYearModel> = { filter: [] };
        ...
        it('should return mapped values when Prisma returns results', async () => {
            ...
            const modelResult = { id: 'id-123' } as unknown as OrderYearModel;

            prisma.orderYearModel.findMany.mockResolvedValue(prismaResults);
            jest.spyOn(OrderYearModelMapper, "toModel").mockReturnValue(modelResult);
            ...
            expect(prisma.orderYearModel.findMany).toHaveBeenCalledWith({ where: mockPrismaWhere });
        ...
        it('should return empty array when Prisma returns no results', async () => {
            prisma.orderYearModel.findMany.mockResolvedValue([]);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.orderYearModel.findMany.mockRejectedValue(new Error('DB error'));
    ...
    describe('findOneAsync', () => {
        const mockFilter: Filter<OrderYearModel>[] = [];
        const mockPrismaWhere = { name: 'value1' };
        ...
        it('should return mapped value when Prisma returns result', async () => {
            ...
            prisma.orderYearModel.findFirst.mockResolvedValue(prismaResult);
            jest.spyOn(OrderYearModelMapper, "toModel").mockReturnValue(modelResult);

            const result = await repository.findOneAsync(mockFilter);

            expect(prisma.orderYearModel.findFirst).toHaveBeenCalledWith({ where: mockPrismaWhere });
            expect(result).toBe(modelResult);
        });

        it('should return null when Prisma returns null', async () => {
            prisma.orderYearModel.findFirst.mockResolvedValue(null);
            expect(PrismaFilterMapper.map).toHaveBeenCalledWith(mockFilter);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.orderYearModel.findFirst.mockRejectedValue(new Error('Oops'));
