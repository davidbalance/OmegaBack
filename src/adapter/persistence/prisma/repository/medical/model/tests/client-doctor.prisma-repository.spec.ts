...
import { ClientDoctorPrismaRepository } from "../client-doctor.prisma-repository";
import { ClientDoctorModel } from "@omega/medical/core/model/client/client-doctor.model";
import { ClientDoctorModelMapper } from "@omega/adapter/persistence/prisma/mapper/medical/model/client-doctor.model-mapper";

describe('ClientDoctorPrismaRepository', () => {
    let prisma;
    let repository: ClientDoctorPrismaRepository;
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ClientDoctorPrismaRepository,
                { provide: PrismaService, useValue: prisma }
            ]
        }).compile();

        repository = module.get<ClientDoctorPrismaRepository>(ClientDoctorPrismaRepository);
    });
    ...
    describe('countAsync', () => {
        const mockFilter: Filter<ClientDoctorModel>[] = [];
        ...
        it('should return the count from Prisma', async () => {
            prisma.clientDoctorModel.count.mockResolvedValue(3);

            const result = await repository.countAsync(mockFilter);

            expect(PrismaFilterMapper.map).toHaveBeenCalledWith(mockFilter);
            expect(prisma.clientDoctorModel.count).toHaveBeenCalledWith({ where: mockPrismaWhere });
            expect(result).toBe(3);
        });

        it("should throw RepositoryError if Prisma throws", async () => {
            prisma.clientDoctorModel.count.mockRejectedValue(new Error("DB error"));
    ...
    describe('findManyAsync', () => {
        const mockFilter: SearchCriteria<ClientDoctorModel> = { filter: [] };
        const mockPrismaWhere = { name: 'value1' };
        ...
        it('should return mapped values when Prisma returns results', async () => {
            ...
            const modelResult = { id: 'id-123' } as unknown as ClientDoctorModel;

            prisma.clientDoctorModel.findMany.mockResolvedValue(prismaResults);
            jest.spyOn(ClientDoctorModelMapper, "toModel").mockReturnValue(modelResult);
            ...
            expect(prisma.clientDoctorModel.findMany).toHaveBeenCalledWith({
        ...
        it('should return empty array when Prisma returns no results', async () => {
            prisma.clientDoctorModel.findMany.mockResolvedValue([]);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.clientDoctorModel.findMany.mockRejectedValue(new Error('DB error'));
    ...
    describe('findOneAsync', () => {
        ...
        it('should return mapped value when Prisma returns result', async () => {
            ...
            expect(PrismaFilterMapper.map).toHaveBeenCalledWith(mockFilter);
            prisma.clientDoctorModel.findFirst.mockResolvedValue(prismaResult);
            jest.spyOn(ClientDoctorModelMapper, "toModel").mockReturnValue(modelResult);

            const result = await repository.findOneAsync(mockFilter);

            expect(prisma.clientDoctorModel.findFirst).toHaveBeenCalledWith({ where: mockPrismaWhere });
            expect(result).toBe(modelResult);
        });

        it('should return null when Prisma returns null', async () => {
            prisma.clientDoctorModel.findFirst.mockResolvedValue(null);
            expect(PrismaFilterMapper.map).toHaveBeenCalledWith(mockFilter);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.clientDoctorModel.findFirst.mockRejectedValue(new Error('Oops'));
