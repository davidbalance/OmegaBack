import { TestModel as PrismaModel } from "@prisma/client";
...
import { TestModelMapper } from "@omega/adapter/persistence/prisma/mapper/medical/model/test.model-mapper";

describe('TestPrismaRepository', () => {
    let prisma: { testModel: any; };
    ...
    describe('findManyAsync', () => {
        const mockFilter: SearchCriteria<TestModel> = { filter: [] };
        ...
        it('should return mapped values when Prisma returns results', async () => {
            ...
            const modelResult = { id: 'id-123' } as unknown as TestModel;

            prisma.testModel.findMany.mockResolvedValue(prismaResults);
            jest.spyOn(TestModelMapper, "toModel").mockReturnValue(modelResult);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.testModel.findMany.mockRejectedValue(new Error('DB error'));
    ...
    describe('findOneAsync', () => {
        ...
        it('should return mapped value when Prisma returns result', async () => {
            ...
            prisma.testModel.findFirst.mockResolvedValue(prismaResult);
            jest.spyOn(TestModelMapper, "toModel").mockReturnValue(modelResult);

            const result = await repository.findOneAsync(mockFilter);

            expect(prisma.testModel.findFirst).toHaveBeenCalledWith({ where: mockPrismaWhere });
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.testModel.findFirst.mockRejectedValue(new Error('Oops'));
