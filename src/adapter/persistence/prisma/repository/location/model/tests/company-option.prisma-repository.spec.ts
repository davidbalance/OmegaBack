import { CompanyOptionModel as PrismaModel } from "@prisma/client";
...
import { CompanyOptionPrismaRepository } from "../company-option.prisma-repository";
import { CompanyOptionModel } from "@omega/location/core/models/corporative/company-option.model";
import { CompanyOptionModelMapper } from "@omega/adapter/persistence/prisma/mapper/location/model/company-option.model-mapper";

describe('CompanyOptionPrismaRepository', () => {
    let prisma: { companyOptionModel: any; };
    let repository: CompanyOptionPrismaRepository;
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                CompanyOptionPrismaRepository,
                { provide: PrismaService, useValue: prisma }
            ]
        }).compile();

        repository = module.get<CompanyOptionPrismaRepository>(CompanyOptionPrismaRepository);
    });
    ...
    describe('findManyAsync', () => {
        const mockFilter: SearchCriteria<CompanyOptionModel> = { filter: [{ field: 'companyLabel', operator: 'eq', value: 'value1' }] };
        ...
        it('should return mapped values when Prisma returns results', async () => {
            ...
            const modelResult = { id: 'id-123' } as unknown as CompanyOptionModel;

            prisma.companyOptionModel.findMany.mockResolvedValue(prismaResults);
            jest.spyOn(CompanyOptionModelMapper, "toModel").mockReturnValue(modelResult);
            ...
            expect(prisma.companyOptionModel.findMany).toHaveBeenCalledWith({ where: mockPrismaWhere });
        ...
        it('should return empty array when Prisma returns no results', async () => {
            prisma.companyOptionModel.findMany.mockResolvedValue([]);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.companyOptionModel.findMany.mockRejectedValue(new Error('DB error'));
    ...
    describe('findOneAsync', () => {
        const mockFilter: Filter<CompanyOptionModel>[] = [{ field: 'companyLabel', operator: 'eq', value: 'value1' }];
        const mockPrismaWhere = { name: 'value1' };
        ...
        it('should return mapped value when Prisma returns result', async () => {
            ...
            prisma.companyOptionModel.findFirst.mockResolvedValue(prismaResult);
            jest.spyOn(CompanyOptionModelMapper, "toModel").mockReturnValue(modelResult);

            const result = await repository.findOneAsync(mockFilter);

            expect(prisma.companyOptionModel.findFirst).toHaveBeenCalledWith({ where: mockPrismaWhere });
            expect(result).toBe(modelResult);
        });

        it('should return null when Prisma returns null', async () => {
            prisma.companyOptionModel.findFirst.mockResolvedValue(null);
            expect(PrismaFilterMapper.map).toHaveBeenCalledWith(mockFilter);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.companyOptionModel.findFirst.mockRejectedValue(new Error('Oops'));
