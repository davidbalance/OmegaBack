...
import { JobPositionModelMapper } from "@omega/adapter/persistence/prisma/mapper/location/model/job-position.model-mapper";
...
describe('JobPositionPrismaRepository', () => {
    ...
    describe('countAsync', () => {
        const mockFilter: Filter<JobPositionModel>[] = [];
        ...
        it('should return the count from Prisma', async () => {
            prisma.jobPositionModel.count.mockResolvedValue(3);

            const result = await repository.countAsync(mockFilter);

            expect(PrismaFilterMapper.map).toHaveBeenCalledWith(mockFilter);
            expect(prisma.jobPositionModel.count).toHaveBeenCalledWith({ where: mockPrismaWhere });
            expect(result).toBe(3);
        });

        it("should throw RepositoryError if Prisma throws", async () => {
            prisma.jobPositionModel.count.mockRejectedValue(new Error("DB error"));
    ...
    describe('findManyAsync', () => {
        const mockFilter: SearchCriteria<JobPositionModel> = { filter: [] };
        const mockPrismaWhere = { name: 'value1' };
        ...
        it('should return mapped values when Prisma returns results', async () => {
            ...
            const modelResult = { id: 'id-123' } as unknown as JobPositionModel;

            prisma.jobPositionModel.findMany.mockResolvedValue(prismaResults);
            jest.spyOn(JobPositionModelMapper, "toModel").mockReturnValue(modelResult);
            ...
            expect(prisma.jobPositionModel.findMany).toHaveBeenCalledWith({
        ...
        it('should return empty array when Prisma returns no results', async () => {
            prisma.jobPositionModel.findMany.mockResolvedValue([]);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.jobPositionModel.findMany.mockRejectedValue(new Error('DB error'));
    ...
    describe('findOneAsync', () => {
        ...
        it('should return mapped value when Prisma returns result', async () => {
            ...
            expect(PrismaFilterMapper.map).toHaveBeenCalledWith(mockFilter);
            prisma.jobPositionModel.findFirst.mockResolvedValue(prismaResult);
            jest.spyOn(JobPositionModelMapper, "toModel").mockReturnValue(modelResult);

            const result = await repository.findOneAsync(mockFilter);

            expect(prisma.jobPositionModel.findFirst).toHaveBeenCalledWith({ where: mockPrismaWhere });
            expect(result).toBe(modelResult);
        });

        it('should return null when Prisma returns null', async () => {
            prisma.jobPositionModel.findFirst.mockResolvedValue(null);
            expect(PrismaFilterMapper.map).toHaveBeenCalledWith(mockFilter);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.jobPositionModel.findFirst.mockRejectedValue(new Error('Oops'));
