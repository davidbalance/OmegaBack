import { AreaOptionModel as PrismaModel } from "@prisma/client";
import { AreaOptionPrismaRepository } from "../area-option.prisma-repository";
...
import { AreaOptionModelMapper } from "@omega/adapter/persistence/prisma/mapper/location/model/area-option.model-mapper";
import { RepositoryError } from "@shared/shared/domain/error";

describe('AreaOptionPrismaRepository', () => {
    let prisma: { areaOptionModel: any; };
    let repository: AreaOptionPrismaRepository;
    ...
    beforeEach(async () => {
        ...
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                AreaOptionPrismaRepository,
                { provide: PrismaService, useValue: prisma }
            ]
        }).compile();

        repository = module.get<AreaOptionPrismaRepository>(AreaOptionPrismaRepository);
    });
    ...
    describe('findManyAsync', () => {
        const mockFilter: SearchCriteria<AreaOptionModel> = { filter: [{ field: 'areaLabel', operator: 'eq', value: 'value1' }] };
        ...
        it('should return mapped values when Prisma returns results', async () => {
            ...
            const modelResult = { id: 'id-123' } as unknown as AreaOptionModel;

            prisma.areaOptionModel.findMany.mockResolvedValue(prismaResults);
            jest.spyOn(AreaOptionModelMapper, "toModel").mockReturnValue(modelResult);
            ...
            expect(prisma.areaOptionModel.findMany).toHaveBeenCalledWith({ where: mockPrismaWhere });
        ...
        it('should return empty array when Prisma returns no results', async () => {
            prisma.areaOptionModel.findMany.mockResolvedValue([]);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.areaOptionModel.findMany.mockRejectedValue(new Error('DB error'));
    ...
    describe('findOneAsync', () => {
        const mockFilter: Filter<AreaOptionModel>[] = [{ field: 'areaLabel', operator: 'eq', value: 'value1' }];
        const mockPrismaWhere = { name: 'value1' };
        ...
        it('should return mapped value when Prisma returns result', async () => {
            ...
            prisma.areaOptionModel.findFirst.mockResolvedValue(prismaResult);
            jest.spyOn(AreaOptionModelMapper, "toModel").mockReturnValue(modelResult);

            const result = await repository.findOneAsync(mockFilter);

            expect(prisma.areaOptionModel.findFirst).toHaveBeenCalledWith({ where: mockPrismaWhere });
            expect(result).toBe(modelResult);
        });

        it('should return null when Prisma returns null', async () => {
            prisma.areaOptionModel.findFirst.mockResolvedValue(null);
            expect(PrismaFilterMapper.map).toHaveBeenCalledWith(mockFilter);
        ...
        it('should throw RepositoryError when Prisma throws', async () => {
            prisma.areaOptionModel.findFirst.mockRejectedValue(new Error('Oops'));
