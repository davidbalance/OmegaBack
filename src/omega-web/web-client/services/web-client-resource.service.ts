...
import { WebResource } from '@/omega-web/web-resource/dtos/response/web-resource.base.dto';
...
export class WebClientResourceService {
  ...
  async find(user: number): Promise<WebResource[]> {
  ...
  async updateResources(user: number, { resources }: PatchWebClientResourceRequestDto): Promise<void> {
    const webResources: any = await this.resourceService.findInIds(resources);
