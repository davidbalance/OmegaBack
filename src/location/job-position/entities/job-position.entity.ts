...
import { JobPositionExternalKeyEntity } from "./job-position-external-key.entity";

@Entity('tbl_lo_job_position')
@Index('idx_job_position_name', ['name'], { unique: true })
export class JobPositionEntity extends AbstractEntity<number> {
    ...
    @OneToOne(() => JobPositionExternalKeyEntity, { eager: false, nullable: true })
    @JoinColumn({ foreignKeyConstraintName: 'fk_lo_external_job_position', referencedColumnName: 'id', name: 'external_key' })
    public readonly externalKey: JobPositionExternalKeyEntity;
}