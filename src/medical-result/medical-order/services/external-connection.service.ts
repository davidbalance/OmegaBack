...
export class ExternalConnectionService {
     ...
     * @returns Order
     */
    async create({ key, source, branch, patient, ...order }: POSTMedicalOrderRequestDTO & { source: string }): Promise<MedicalOrder> {
     ...
     * @returns Order
     */
    async findOneOrCreate({ key, source, branch, patient, ...order }: POSTMedicalOrderRequestDTO & { source: string }): Promise<MedicalOrder> {
        try {
     ...
     * @returns Order
     */
    async findOneAndUpdate({ key, source }: { key: string, source: string }, { ...data }: PATCHMedicalOrderRequestDTO): Promise<MedicalOrder> {
