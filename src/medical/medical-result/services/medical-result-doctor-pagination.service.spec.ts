...
import { MedicalResultDoctorPaginationService } from "./medical-result-doctor-pagination.service";
import { mockMedicalResultEntities, mockMedicalResultEntity } from "../stub/medical-result-entity.stub";

describe('MedicalResultDoctorPaginationService', () => {
  let service: MedicalResultDoctorPaginationService;
  let repository: jest.Mocked<MedicalResultRepository>;

  beforeEach(async () => {
    const { unit, unitRef } = TestBed.create(MedicalResultDoctorPaginationService).compile();
  ...
  describe('queryBuilder', () => {
    ...
    const page = 0;
    const take = 10;
    const extras: { doctor: string, order: number } = {
      doctor: "1234567890",
      order: 1
    };
    ...
    it('should build a query', async () => {
      ...
      expect(repository.query().innerJoin).toHaveBeenCalledWith('result.order', 'order', 'order.id = :order', { order: extras.order });
      ...
      expect(repository.query().where).toHaveBeenCalledWith('result.doctorDni LIKE :doctor', { doctor: extras.doctor });
      expect(repository.query().andWhere).toHaveBeenCalledWith('result.examName LIKE :examName', { examName: `%${search}%` });
