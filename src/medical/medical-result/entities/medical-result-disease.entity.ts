...
import { MedicalResultEntity } from "./medical-result.entity";

@Entity('tbl_m_result_diseases')
export class MedicalResultDiseaseEntity extends AbstractEntity<number> {
    ...
    @ManyToOne(() => MedicalResultEntity, result => result.diseases, { eager: false, nullable: false })
    @JoinColumn({ foreignKeyConstraintName: 'fk_m_result_disease', referencedColumnName: 'id', name: 'result_id' })
    public result: MedicalResultEntity;
