...
import { MedicalOrderDoctorPaginationService } from "./medical-order-doctor-pagination.service";
import { mockMedicalOrderEntities } from "../stubs/medical-order-entity.stub";

describe('MedicalOrderDoctorPaginationService', () => {
  let service: MedicalOrderDoctorPaginationService;
  let repository: jest.Mocked<MedicalOrderRepository>;

  beforeEach(async () => {
    const { unit, unitRef } = TestBed.create(MedicalOrderDoctorPaginationService).compile();
  ...
  describe('queryBuilder', () => {
    ...
    it('should build a query', async () => {
      ...
      expect(repository.query().innerJoin).toHaveBeenCalledWith('order.results', 'result', 'result.doctorDni = :doctor', { doctor: extras.doctor });
      expect(repository.query().innerJoin).toHaveBeenCalledWith('order.client', 'client', 'client.dni = :patient', { patient: extras.patient });
      expect(repository.query().leftJoinAndSelect).toHaveBeenCalledWith('result.report', 'report');
      ...
      expect(repository.query().addSelect).toHaveBeenNthCalledWith(5, 'report.id', 'report');
